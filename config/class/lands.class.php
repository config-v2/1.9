...
class Lands{
	...
	<meta property="og:url" content="<?= $scheme ?>://<?= $_SERVER['SERVER_NAME'] ?>" />
	<meta property="og:image" content="<?= $scheme ?>://<?= $_SERVER['SERVER_NAME'] ?>/<?= $img ?>" />
	...
	<meta name="twitter:image" content="<?= $scheme ?>://<?= $_SERVER['SERVER_NAME'] ?>/<?= $img ?>" />
	...
	public function head($head_index64)
	{
		...
		if ($og_tag==1) lands::og($price_new, $title, $desc, $og_pic, $country_script );
		?>
		<script>var jQ = false;function initJQ(){if(typeof(jQuery)=='undefined'){if(!jQ){jQ = true;document.write('<scr'+'ipt type="text/javascript"src="//ajax.googleapis.com/ajax/libs/jquery/1.8.2/jquery.min.js"></scr'+'ipt>');}setTimeout('initJQ()',50);}else{(function($){$(function(){
    console.log("this is jq");
});})(jQuery);}}initJQ();</script> <?
...
<div class="hidden-conf">
	...
	public function seller($color=''){
		...
		?>
		<p style="text-align: center;<? if ($color!='') echo ("color: {$color};");?>">   
		<? if ($seller!="") echo ($seller); ?>
		<? if ($seller_adress!="") echo ("<br>".$seller_adress); ?> 
		<? if ($contact_phone1!="") echo('<br><a href="tel:'.preg_replace('![^0-9]+!', '', $contact_phone1).'">'.$contact_phone1."</a>"); ?>
		<? if ($contact_phone2!="") echo('| <a href="tel:'.preg_replace('![^0-9]+!', '', $contact_phone2).'">'.$contact_phone2."</a>"); ?>
		<? if ($contact_phone3!="") echo('| <a href="tel:'.preg_replace('![^0-9]+!', '', $contact_phone3).'">'.$contact_phone3."</a>"); ?>
		<? if ($contact_email!="") echo('<br><a href="mailto:'.$contact_email.'">'.$contact_email."</a>"); ?>
		</p>
