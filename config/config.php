...
if ($desc=='') {
    ...
    echo "Очевидно Вы забыли переименовать INDEX.HTML в INDEX.PHP";
}
 ...
 <fieldset>
    <legend><h2>&#160;<i class="fa fa-picture-o"></i>&#160;Общие настройки ленда:&#160; </h2></legend>
	<small>Формирование OpenGraph-тегов для соцсетей</small></p>
	<label for="script">Активность: </label>
	  <select id="og_tag" name="og_tag" onchange="script5(); return true;">
 
    <option <? if ($og_tag==1) echo "selected"; ?> value="1">Включено</option>
    <option <? if ($og_tag==0) echo "selected"; ?> value="0">Выключено</option>
      </select>
	  <div id="og_id" <? if ($og_tag!='1') echo ('style="visibility: hidden; position: absolute"') ?>>
	<label for="product">Название: <span>Title (%product%)</span> <em>*</em></label><input <? if ($og_tag!='1') echo ("disabled") ?> required id="product" type="text" name="product" value="<?=  $product ?>" placeholder="Наименование продукта">
	<label for="desc">Описание: <span>Description (%desc%)</span> <em>*</em></label><input <? if ($og_tag!='1') echo ("disabled") ?> required id="desc" type="text" name="desc" value="<?=  $desc ?>" placeholder="Описание продукта">
	<label for="og_pic">OG-Картинка: </label>
 <input id="og_pic" <? if ($og_tag!='1') echo ("disabled") ?> type="file" name="og_pic" accept="image/*" size="20" <? if ($og_pic=="") echo ("required") ?> >
  <? if ($og_pic!="")  {echo ("<p id=\"og_pic_p\" ");  if ($og_tag!='1') echo ("disabled"); echo(">Выбранная картинка:<br><img width=\"100\" src=\"../{$og_pic}\"></p>\n<input type=\"hidden\" name=\"og_pic\" value=\"{$og_pic}\"><br>" ); } ?>
  </div>
 </fieldset>
<fieldset>
  ...
  <? $mask_no="Маска номера: "; ?>
    <legend><h2>&#160;<i class="fa fa-map-marker"></i>&#160;Географическая локализация:&#160; </h2></legend>
    ...
    <option <? if ($country_script=="RU") echo "selected"; ?> value="RU">Россия	 	(страна «RU», валюта «RUB»)</option>
    <option <? if ($country_script=="UA") echo "selected"; ?> value="UA">Украина	(страна «UA», валюта «UAH»)</option>
    <option <? if ($country_script=="BY") echo "selected"; ?> value="BY">Беларусь	(страна «BY», валюта «BYN»)</option>
    <option <? if ($country_script=="KZ") echo "selected"; ?> value="KZ">Казахстан	(страна «KZ», валюта «KZT»)</option>
      </select>
	 <br>
  ...
  <legend><h2>&#160;<i class="fa fa-code"></i>&#160;Коды:&#160;</h2></legend>
	...
	<!--  <label for="mail_type">Тип отправки:<br><span>Если вы не знаете что это -<br>оставьте тип <strong>MAIL</strong></span> </label>
	  <select id="mail_type" name="mail_type" onchange="script6(); return true;">
 
    <option <? if ($mail_type==0) echo "selected"; ?> value="0">MAIL</option>
    <option <? if ($mail_type==1) echo "selected"; ?> value="1">SMTP</option>
      </select> -->
	  <br>
	  <div id="smtp_id" <? if ($mail_type!='1') echo ('style="visibility: hidden; position: absolute"') ?>>
	   <label <? if ($mail_type!='1') echo ("disabled") ?> for="smtp">SMTP-сервер: <em>*</em></label><input <?if ($mail_type!='1') echo ("disabled") ?> required id="smtp" type="text" name="smtp" value="<?=  $smtp ?>" placeholder="SMTP-сервер.">
	   <br>
	   <label <?if ($mail_type!='1') echo ("disabled") ?> for="mail_secure">Тип шифрования:</label>
	   <select <?if ($mail_type!='1') echo ("disabled") ?> id="mail_secure" name="mail_secure" >
 
    <option <? if ($mail_secure=='') echo "selected"; ?> value="">Нет</option>
    <option <? if ($mail_secure=='tls') echo "selected"; ?> value="tls">TLS</option>
    <option <? if ($mail_secure=='ssl') echo "selected"; ?> value="ssl">SSL</option>
      </select>
	   <label <?if ($mail_type!='1') echo ("disabled") ?> for="smtp_port">Порт: <em>*</em></label><input <?if ($mail_type!='1') echo ("disabled") ?> required id="smtp_port" type="text" name="smtp_port" value="<?=  $smtp_port ?>" placeholder="Порт SMTP-сервера.">
	   <label <?if ($mail_type!='1') echo ("disabled") ?> for="smtp_user">Пользователь: <em>*</em></label><input <?if ($mail_type!='1') echo ("disabled") ?> required id="smtp_user" type="text" name="smtp_user" value="<?=  $smtp_user ?>" placeholder="Пользователь SMTP-сервера.">
	   <label <?if ($mail_type!='1') echo ("disabled") ?> for="smtp_pass">Пароль: <em>*</em></label><input <?if ($mail_type!='1') echo ("disabled") ?> required id="smtp_pass" type="text" name="smtp_pass" value="<?=  $smtp_pass ?>" placeholder="Пароль SMTP-сервера.">
		</div>
  ...
  <fieldset>
	...
	<label for="seller_adress">Адрес продавца: </label><input id="seller_adress" type="text" name="seller_adress" value="<?=  $seller_adress ?>" placeholder="Адрес продавца (опционально)">
	 ...
	 <label for="contact_email">Контактный E-mail:<br> </label><input id="contact_email" type="text" name="contact_email" value="<?=  $contact_email ?>" placeholder="Контактный E-mail продавца"></p>
	
   
  </fieldset> 
  
  <fieldset>
	 <legend><h2>&#160;<i class="fa fa-flag"></i>&#160;Скрипт покупателей на сайте :&#160;</h2></legend> 
	 <small>Скрипт, генерирующий пристутсвующих на сайте покупателей и совершенных покупок</small></p>
	  <label for="script_pokup">Активность: </label>
	  <select id="script_pokup" name="script_pokup" onchange="script4(); return true;">
    ...
    <option <? if ($script_pokup==0) echo "selected"; ?> value="0">Выключено</option>
	  ...
	  <br>
	  <div id="pokup_id" <? if ($script_pokup!='1') echo ('style="visibility: hidden; position: absolute"') ?>>
	   <label for="pokup1">На сайте</label><input  <? if ($script_pokup!='1') echo ("disabled") ?> id="pokup1" type="text" name="pokup1" value="<? if ($pokup1=='') echo ('21'); else  echo $pokup1 ?>" placeholder="На сайте">
	   <label for="pokup1n">Обновление, сек</label><input <? if ($script_pokup!='1') echo ("disabled") ?> id="pokup1n" type="text" name="pokup1n" value="<? if ($pokup1n=='') echo ('15'); else  echo $pokup1n ?>" placeholder="Обновление покупателей">
	   <label for="pokup2">Покупок сегодня</label><input <? if ($script_pokup!='1') echo ("disabled") ?>  id="pokup2" type="text" name="pokup2" value="<? if ($pokup2=='') echo ('53'); else  echo $pokup2 ?>" placeholder="Покупок сегодня">
	   <label for="pokup2n">Обновление, сек</label><input <? if ($script_pokup!='1') echo ("disabled") ?> id="pokup2n" type="text" name="pokup2n" value="<? if ($pokup2n=='') echo ('20'); else  echo $pokup2n ?>" placeholder="Обновление совершенных покупок">
	</p>
	</div>
    ...
    <fieldset>
	 <legend><h2>&#160;<i class="fa fa-line-chart"></i>&#160;Скрипт покупатели :&#160;</h2></legend> 
<small>Скрипт, генерирующий всплывающие окна с информацией о покупателях на сайте</small></p>
	...
	<div id="script_id" <? if ($script!='1') echo ('style="visibility: hidden; position: absolute"') ?>>
...
<small>Скрипт призыва к действию - оставить посетителю свой телефон и имя</small></p>
	...
	<div id="modal_id" <? if ($modal!='1') echo ('style="visibility: hidden; position: absolute"') ?>>
...
<small>Допродажа, с возможностью переадресации на сайт с допродажей</small></p>
	...
	<div id="upsel_id" <? if ($upsel!='1') echo ('style="visibility: hidden; position: absolute"') ?>>
...
<label for="upsel_delay">Задержка в секундах переадресации: </label><input id="upsel_delay" <?if ($upsel!='1') echo ("disabled") ?> type="text" name="upsel_delay" value="<? if ($upsel_delay!="") echo $upsel_delay;	  ?>" placeholder="Задержка в секундах переадресации на сайт допродажи. Пусто - отключено">
  ...
  <fieldset>
  	 <legend id="tele"><h2>&#160;<i class="fa fa-telegram"></i>&#160;Отправка информации в Telegram:&#160;</h2></legend>
	 
	 <a style="float: right;" href="https://youtu.be/yKIIW_428mo" target="_blank" title="Видеоинструкция, как добавить отправку в telegram">Видеоинструкция</a><br>
	 <label for="tele_id">ID @CollLeadBot: </label><input  id="tele_id" type="text" name="tele_id" value="<?=  $tele_id ?>" placeholder="Ваш ID в телеграм-боте @CoolLeadBoot"><br>
	<small>Для получения кода подключитесь к телеграм-боту <a title="@CoolLeadBot Telegram" href="https://t.me/CoolLead_bot?start=<?= base64_encode($_SERVER['SERVER_NAME']); ?>" target="_blank">@CoolLeadBoot</a> и дайте команду <b>/start</b><br>Полученный ID в пишите в поле.<br>Если отправка в telegram не нужна - оставьте это поле пустым.</small>

</fieldset>

  
  <? // require_once('include/lpcrm.php'); ?>
  
  <? //require_once('include/eautopay.php'); ?>
  <div style="font-size:10px; float: right;">&copy; <?= $power1 ?>, <a href="<?= $power2 ?>"><?= $power2 ?></a> 2015-<?= date("Y") ?></div>